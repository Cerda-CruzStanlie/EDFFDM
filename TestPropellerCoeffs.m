% The goal of this script is to test PropellerCoeffs.m at 55% throttle with
% 2815 + 11x5.5 in pitch with a 4s battery.
% Q: Do the equation set for constants derivations actually work?
clc; clear;

%% OUTPUTS AT 55% throttle -----------------------------------------------------------

% givens
Kv         = 900;   % RPM per volt
R          = 11/2*0.0254; % Blade radius (m)
Pitch_in   = 5.5;    % Geometric pitch (in)
rho        = 1.225;  % Air density (kg/m^3)

%Thrust Factors
V          = 15.44;   % Battery voltage  (V)
I          = 8.52;     % Current (A)
loaded_factor = 0.5953;
eta_e = 0.7334;
CT_static = 9.7200;
eta_p = .7481; %Probably eta_prop
